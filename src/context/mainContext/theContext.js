...
var desc_sorted_dates_string = []
  ...
  sorted_results.sort(function(a,b){ // Using Comparison function of JS to sort dates array
    return a-b
  })

  desc_sorted_dates.sort(function(a,b){ // Again Using comaparison function of JS to sorts dates array in reverse order.
    return b-a
  })
  ...
  const makeNameGraph = async(login) =>{
    ...
    filteredResponse.map((eachResponse2)=> TCost += parseFloat(eachResponse2.Cost))
    filteredResponse.map((eachResponse2)=> TConQuan += parseFloat(eachResponse2.ConsumedQuantity))
    costSet.push(TCost)  
    cqSet.push(TConQuan)
    TCost = 0
    TConQuan = 0
...
const makeNameGraphResources = async(login2) =>{
  ...
  const clearFilterSearch = () => {sdispatch({ type: "CLEAR_FILTERS" })}
  const clearFilterResources = () => dispatch({type: "CLEAR_FILTERS_RESOURCE"})
